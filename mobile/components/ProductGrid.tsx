  ...
  const handleAddToCart = (productId:string, productName:string)=>{
   addToCart(
    {productId, quantity:1},
    {
      ...
      onError:(error:any)=>{
        Alert.alert("Error",error?.response?.data?.error || "Failed to add product to cart. Please try again. ");
      },
    }
   );
  };
      ...
      >
        {isAddingToWishlist || isRemovingFromWishlist ? (
            ...
            <Ionicons
              ...
              size={20}
              color={isInWishlist(product._id) ? "#ff4444" : "#fff"}
            />
...
function NoProductFound(){
  return(
    <View className = "py-20 items-center justify-center">
      ...
      <Text className = "text-text-secondary mt-2">Try adjusting your search terms</Text>
    </View>
  )
}